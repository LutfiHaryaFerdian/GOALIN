...
<header class="sticky top-0 z-50 bg-white border-b border-[#e5e5e5]" x-data="{ mobileOpen: false }">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex h-16 items-center justify-between">
            ...
            <nav class="hidden md:flex items-center gap-8">
                <a href="{{ route('fields.index') }}" class="text-sm font-medium text-[#737373] hover:text-[#0a0a0a] transition-colors">Lapangan</a>
                @auth
                    <a href="{{ route('bookings.index') }}" class="text-sm font-medium text-[#737373] hover:text-[#0a0a0a] transition-colors">Pemesanan</a>
                    @if(auth()->user()->isOwner())
                        <a href="{{ route('owner.dashboard') }}" class="text-sm font-medium text-[#737373] hover:text-[#0a0a0a] transition-colors">Dashboard</a>
                    @endif
                    @if(auth()->user()->isAdmin())
                        <a href="{{ route('admin.dashboard') }}" class="text-sm font-medium text-[#737373] hover:text-[#0a0a0a] transition-colors">Admin</a>
                    @endif
                @endauth
            </nav>
            <div class="flex items-center gap-3">
                ...
                @endauth
                <button @click="mobileOpen = !mobileOpen" class="md:hidden p-2 text-[#737373] hover:text-[#0a0a0a] rounded-lg">
    ...
    <div x-show="mobileOpen" x-transition class="md:hidden border-t border-[#e5e5e5] bg-white px-4 py-3 space-y-1">
        <a href="{{ route('fields.index') }}" class="block px-3 py-2.5 text-sm font-medium text-[#404040] hover:bg-[#f5f5f5] rounded-xl">Lapangan</a>
        @auth
            <a href="{{ route('bookings.index') }}" class="block px-3 py-2.5 text-sm font-medium text-[#404040] hover:bg-[#f5f5f5] rounded-xl">Pemesanan</a>
        @else
            <a href="{{ route('login') }}" class="block px-3 py-2.5 text-sm font-medium text-[#404040] hover:bg-[#f5f5f5] rounded-xl">Masuk</a>
            <a href="{{ route('register') }}" class="block px-3 py-2.5 text-sm font-semibold text-center text-[#0a0a0a] bg-[#f5f5f5] rounded-xl">Daftar Gratis</a>
        @endauth
    </div>
</header>
...
<section class="bg-white min-h-[90vh] flex flex-col justify-center relative overflow-hidden pitch-lines">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-32">
        ...
        {{-- Headline --}}
        <h1 class="font-display text-[4.5rem] sm:text-[6rem] md:text-[8rem] lg:text-[10rem] font-black uppercase leading-none tracking-tighter text-[#0a0a0a] max-w-4xl">
            BOOK YOUR<br>
            <span class="relative inline-block">
                FIELD.
                <span class="absolute -bottom-2 left-0 right-0 h-2 md:h-3 bg-[#16a34a] rounded-full"></span>
            </span>
        </h1>

        {{-- Sub --}}
        <p class="mt-10 text-lg text-[#404040] max-w-md leading-relaxed">
        ...
        <div class="flex flex-wrap items-center gap-8 mt-16 pt-8 border-t border-[#e5e5e5]">
            <div>
                <p class="font-display text-3xl font-bold text-[#0a0a0a]">{{ $totalFields }}+</p>
                <p class="text-xs text-[#737373] uppercase tracking-widest mt-0.5">Lapangan</p>
            </div>
            <div class="w-px h-8 bg-[#e5e5e5]"></div>
            <div>
                <p class="font-display text-3xl font-bold text-[#0a0a0a]">{{ $totalCities }}</p>
                <p class="text-xs text-[#737373] uppercase tracking-widest mt-0.5">Kota</p>
            </div>
            <div class="w-px h-8 bg-[#e5e5e5]"></div>
            <div>
                <p class="font-display text-3xl font-bold text-[#0a0a0a]">{{ $totalBookings }}+</p>
                <p class="text-xs text-[#737373] uppercase tracking-widest mt-0.5">Booking</p>
            </div>
        </div>
    </div>
</section>
...
<section class="bg-[#f8f8f6] py-20 md:py-24">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <p class="section-label mb-2">JENIS LAPANGAN</p>
        <h2 class="font-display text-3xl md:text-4xl font-bold uppercase text-[#0a0a0a] mb-10">Semua Olahraga,<br>Satu Platform.</h2>

        <div class="flex flex-wrap gap-3">
            <a href="{{ route('fields.index') }}"
               class="inline-flex items-center gap-2 px-5 py-2.5 border border-[#e5e5e5] rounded-full text-sm font-medium text-[#0a0a0a] bg-white hover:bg-[#0a0a0a] hover:text-white hover:border-[#0a0a0a] transition-colors duration-200">
            ...
            @foreach($categories as $cat)
                <a href="{{ route('fields.index', ['category' => $cat->slug]) }}"
                   class="inline-flex items-center gap-2 px-5 py-2.5 border border-[#e5e5e5] rounded-full text-sm font-medium text-[#404040] bg-white hover:bg-[#0a0a0a] hover:text-white hover:border-[#0a0a0a] transition-colors duration-200">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-end justify-between mb-12">
            <div>
                ...
                <h2 class="font-display text-3xl md:text-5xl font-bold uppercase text-[#0a0a0a] leading-tight">
                    Temukan lapangan<br>di kotamu.
                </h2>
            </div>
            <a href="{{ route('fields.index') }}" class="hidden sm:flex items-center gap-2 text-sm font-semibold text-[#0a0a0a] hover:text-[#16a34a] transition-colors">
                Lihat Semua
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3"/>
                </svg>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($featuredFields as $field)
                    <a href="{{ route('fields.show', $field->slug) }}"
                       class="group bg-white rounded-2xl overflow-hidden border border-[#e5e5e5] hover:border-[#0a0a0a] transition-colors duration-300 cursor-pointer block">
...
<section id="cara-kerja" class="bg-[#f8f8f6] py-20 md:py-28">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <p class="section-label mb-2">CARA KERJA</p>
        <h2 class="font-display text-3xl md:text-5xl font-bold uppercase text-[#0a0a0a] leading-tight mb-16">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            @foreach([
                ...
                <div class="relative">
                    ...
                    <h3 class="font-display text-2xl font-bold uppercase text-[#0a0a0a] -mt-8">{{ $title }}</h3>
                    <p class="text-sm text-[#737373] mt-2 leading-relaxed max-w-xs">{{ $desc }}</p>
                </div>
            @endforeach
        </div>
    </div>
...
<section class="bg-[#0a0a0a] py-20 md:py-28">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-0 divide-y md:divide-y-0 md:divide-x divide-white/10">
            @foreach([
                ...
                <div class="md:px-12 first:pl-0 last:pr-0 py-8 md:py-0">
                    ...
                    <p class="text-sm uppercase tracking-widest text-[#737373] mt-3">{{ $label }}</p>
                </div>
            @endforeach
        </div>
    </div>
...
<section class="bg-white py-24 md:py-32">
    <div class="max-w-2xl mx-auto px-4 text-center">
        ...
        <h2 class="font-display text-5xl md:text-7xl font-black uppercase text-[#0a0a0a] leading-none mb-6">
            SIAP BERMAIN?
        </h2>
        <p class="text-[#737373] text-base mb-10">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-6">
                ...
                <a href="{{ route('register') }}" class="text-xs text-[#737373] hover:text-[#0a0a0a] transition-colors">Daftar</a>
            </div>
