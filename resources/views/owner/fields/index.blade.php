        ...
        <main class="flex-1 min-w-0 py-8 px-4 sm:px-8">
            ...
            @if ($fields->isEmpty())
                <div class="card p-16 text-center">
                    ...
                    <a href="{{ route('owner.fields.create') }}" class="btn-primary inline-flex">Tambah Lapangan
                        Pertama</a>
                </div>
            @else
                    ...
                    @foreach ($fields as $field)
                        @php
                            $statusColors = [
                                ...
                                'inactive' =>
                                    'bg-gray-100 text-gray-600 rounded-full px-3 py-0.5 text-xs font-semibold',
                        ...
                        <div class="card overflow-hidden">
                            <div class="aspect-video bg-primary-light flex items-center justify-center">
                                ...
                                <img src="{{ $field->firstImage ? asset('storage/' . $field->firstImage) : asset('images/default-field-image.jpg') }}"alt="{{ $field->name }}"
                            ...
                            <div class="p-5">
                                <div class="flex items-start justify-between gap-2 mb-2">
                                    ...
                                    <span
                                        class="{{ $statusColors[$field->status] ?? '' }} shrink-0">{{ ucfirst($field->status) }}</span>
                                </div>
                                <p class="text-xs text-gray-500 mb-1">{{ $field->category->name }} ·
                                    {{ $field->location->city }}</p>
                                <p class="text-sm font-bold text-primary mb-4">Rp
                                ...
                                <div class="grid grid-cols-3 gap-1.5">
                                    ...
                                    <form method="POST" action="{{ route('owner.fields.destroy', $field) }}" x-data
                                        x-on:submit.prevent="if(confirm('Nonaktifkan lapangan ini?')) $el.submit()">
                                        @csrf
                                        @method('DELETE')
                                        <button
                                            class="w-full py-1.5 text-xs font-semibold text-red-500 bg-red-50 rounded-lg hover:bg-red-100 transition-colors">
                                            Nonaktif
                                        </button>
                                    </form>
                                </div>
